                    ...
                    <div className="columns-1 sm:columns-2 md:columns-3 gap-6 space-y-6">
                        <AnimatePresence>
                            {stories.map((story, idx) => {
                                ...
                                return (
                                    <motion.div key={story.id} initial={{opacity: 0, y: 20}} animate={{opacity: 1, y: 0}} transition={{delay: (idx % 10) * 0.1}} className="break-inside-avoid inline-block w-full">
                                        <Card className="overflow-hidden border-2 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 mb-6 bg-card/90 backdrop-blur-sm">
                                            <div className="p-6">
                                                ...
                                                <div className="pt-4 border-t flex items-center gap-4">
                                                    <Button variant="ghost" size="sm" className={`flex items-center gap-2 ${hasSupported ? "text-rose-500 hover:text-rose-600 bg-rose-50" : "text-muted-foreground hover:text-foreground"}`} onClick={() => handleToggleSupport(story.id)}>
                                                        <Heart className={`w-4 h-4 ${hasSupported ? "fill-current" : ""}`} />
                                                        {story.supports?.length || 0} Подкрепи
                                                    </Button>

                                                    <Button variant="ghost" size="sm" className="flex items-center gap-2 text-muted-foreground hover:text-foreground" onClick={() => setExpandedComments((prev) => ({...prev, [story.id]: !prev[story.id]}))}>
                                                        ...
                                                        {user ? (
                                                            <div className="flex gap-2">
                                                                ...
                                                                <div className="flex-1 flex gap-2">
                                                                    <Input placeholder="Напиши коментар..." className="h-8 text-sm" value={commentInputs[story.id] || ""} onChange={(e) => setCommentInputs((prev) => ({...prev, [story.id]: e.target.value}))} onKeyDown={(e) => e.key === "Enter" && submitComment(story.id)} />
                                                            ...
                                                            <div className="text-sm text-muted-foreground text-center pb-2">Трябва да влезете, за да коментирате.</div>
                                                        )}

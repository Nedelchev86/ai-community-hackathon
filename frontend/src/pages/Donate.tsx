...
const Donate = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!isAuthenticated) {
            toast.error("Трябва да сте влезли в профила си, за да дарите.");
            navigate("/login");
            return;
        }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(data.message || "Грешка при създаване на дарението");
            }

            toast.success("Успешно дарено!", {
                description: "Благодарим ви за добрината!",
            });
        ...
        <div className="container py-12 flex justify-center">
            <motion.div initial={{opacity: 0, y: 20}} animate={{opacity: 1, y: 0}} className="w-full max-w-xl">
                <div className="text-center mb-8">
                    <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-green-400 to-green-600 grid place-items-center mx-auto shadow-lg mb-4">
                        <HeartHandshake className="w-8 h-8 text-white" />
                    </div>
                    <h1 className="text-3xl font-bold tracking-tight">Дари вещ</h1>
                    <p className="text-muted-foreground mt-2">Дай втори живот на нещата, които вече не използваш.</p>
                </div>

                <Card>
                    <CardHeader>
                        <CardTitle>Информация за дарението</CardTitle>
                        <CardDescription>Опишете предмета, който искате да дарите на някой в нужда.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div className="space-y-2">
                                <Label htmlFor="title">Заглавие</Label>
                                <Input id="title" name="title" placeholder="напр. Детски дрехи за 5-годишно момче" value={formData.title} onChange={handleChange} required />
                            </div>
                            <div className="space-y-2">
                                <Label htmlFor="description">Описание / Състояние</Label>
                                <Textarea id="description" name="description" placeholder="Опишете състоянието и всякакви важни детайли." value={formData.description} onChange={handleChange} required rows={4} />
                            </div>
                            <div className="space-y-2">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex flex-col gap-3">
                                    ...
                                    <div className="flex gap-2 items-center">
                                        ...
                                        {formData.lat && formData.lng && <span className="text-sm text-green-600 font-medium">✔️ Локацията е избрана</span>}
                                    </div>
                                </div>
                                <p className="text-xs text-muted-foreground">Маркирайте с клик върху картата или използвайте геолокация, за да позволите на другите лесно да я намерят.</p>
                            </div>
                            <div className="space-y-2">
                            ...
                            <Button type="submit" className="w-full bg-green-600 hover:bg-green-700 mt-6" disabled={isLoading}>
                                {isLoading ? "Изпращане..." : "Публикувай дарението"}
                            </Button>
                        </form>
                    </CardContent>
                </Card>
